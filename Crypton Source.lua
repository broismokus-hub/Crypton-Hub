...
local Config = {
    ...
    Aimbot = {
        Enabled=false, Smoothing=1,  -- 1 = instant snap
        FovRadius=150,
...
local AccentElements = {
    sliderFill={}, sliderVal={}, keybindText={}, navBtnRef={},
}
...
-- ============================================================
local M = {
    Win=Color3.fromRGB(236,236,236), Sidebar=Color3.fromRGB(210,210,215),
    TitleBar=Color3.fromRGB(220,220,223), Separator=Color3.fromRGB(185,185,190),
    Row=Color3.fromRGB(236,236,236), RowActive=Color3.fromRGB(9,101,224),
    TextDark=Color3.fromRGB(30,30,30), TextMid=Color3.fromRGB(90,90,100),
    TextLight=Color3.fromRGB(150,150,160), TextWhite=Color3.fromRGB(255,255,255),
    Accent=Color3.fromRGB(9,101,224), Red=Color3.fromRGB(255,59,48),
    Yellow=Color3.fromRGB(255,189,46), Green=Color3.fromRGB(40,200,64),
    Content=Color3.fromRGB(245,245,247), ToggleOff=Color3.fromRGB(190,190,195),
    ToggleOn=Color3.fromRGB(52,199,89), SliderBg=Color3.fromRGB(200,200,205),
    SectionBg=Color3.fromRGB(226,226,230), Border=Color3.fromRGB(170,170,178),
}

local function ApplyAccent(col)
    M.Accent=col; M.RowActive=col
    ...
    for _,info in ipairs(AccentElements.navBtnRef) do
        if info.active then pcall(function() info.btn.BackgroundColor3=col end) end
    end
end

local function R(p,r) local c=Instance.new("UICorner",p); c.CornerRadius=UDim.new(0,r or 6); return c end
local function Stroke(p,col,th) local s=Instance.new("UIStroke",p); s.Color=col or M.Border; s.Thickness=th or 1; return s end
...
-- ============================================================
--  WINDOW
--  FIX: Shadow is a CHILD of Window so it moves with it.
--       It uses a negative ZIndex offset and negative position
--       to sit behind the window content.
-- ============================================================
...
Window.ClipsDescendants = false   -- needed so shadow shows outside bounds
R(Window,12); Stroke(Window,M.Border,1)

-- Shadow is now INSIDE Window at a negative offset, behind everything
...
Shadow.ZIndex           = 1   -- behind window content (Window itself is ZIndex 2)
R(Shadow,16)

-- Title bar
local TitleBar = Instance.new("Frame",Window)
TitleBar.Size=UDim2.new(1,0,0,38); TitleBar.BackgroundColor3=M.TitleBar
...
local espPage=MakePage(function(s)
    ...
    previewRow.BorderSizePixel=0; previewRow.LayoutOrder=13; previewRow.ZIndex=5; R(previewRow,6)
    ...
    Instance.new("Frame",previewBg).Size=UDim2.new(1,0,0,1)
    local g=previewBg:FindFirstChildOfClass("Frame"); if g then g.Position=UDim2.new(0,0,1,-30); g.BackgroundColor3=Color3.fromRGB(60,65,75); g.BorderSizePixel=0; g.ZIndex=6 end
    local skinTone=Color3.fromRGB(255,200,160); local clothTop=Color3.fromRGB(90,130,200); local clothBot=Color3.fromRGB(60,80,150); local shoe=Color3.fromRGB(40,40,40)
    local function AP(px,py,pw,ph,col)
        local p=Instance.new("Frame",previewBg); local avatarH=110; local groundY=30
        p.Size=UDim2.new(0,pw,0,ph); p.Position=UDim2.new(0.5,px-pw/2,1,-(groundY+py+ph))
        p.BackgroundColor3=col; p.BorderSizePixel=0; p.ZIndex=7; R(p,2); return p
    end
    AP(-10,0,12,10,shoe); AP(10,0,12,10,shoe); AP(-10,10,11,22,clothBot); AP(10,10,11,22,clothBot)
    AP(-10,32,12,22,clothBot); AP(10,32,12,22,clothBot); AP(0,54,28,32,clothTop)
    AP(-22,54,10,28,clothTop); AP(22,54,10,28,clothTop); AP(0,86,8,6,skinTone); AP(0,92,24,22,skinTone)
    -- box lines
    local bl=Instance.new("Frame",previewBg); bl.Size=UDim2.new(0,1,0,116); bl.Position=UDim2.new(0.5,-19,0,12); bl.BackgroundColor3=Config.ESP.BoxColor; bl.BorderSizePixel=0; bl.ZIndex=9
    local br=Instance.new("Frame",previewBg); br.Size=UDim2.new(0,1,0,116); br.Position=UDim2.new(0.5,18,0,12); br.BackgroundColor3=Config.ESP.BoxColor; br.BorderSizePixel=0; br.ZIndex=9
    local bt=Instance.new("Frame",previewBg); bt.Size=UDim2.new(0,38,0,1); bt.Position=UDim2.new(0.5,-19,0,12); bt.BackgroundColor3=Config.ESP.BoxColor; bt.BorderSizePixel=0; bt.ZIndex=9
    local bb=Instance.new("Frame",previewBg); bb.Size=UDim2.new(0,38,0,1); bb.Position=UDim2.new(0.5,-19,0,128); bb.BackgroundColor3=Config.ESP.BoxColor; bb.BorderSizePixel=0; bb.ZIndex=9
    local function SetBoxColor(c) bl.BackgroundColor3=c; br.BackgroundColor3=c; bt.BackgroundColor3=c; bb.BackgroundColor3=c end
    local function ShowBox(v) bl.Visible=v; br.Visible=v; bt.Visible=v; bb.Visible=v end
    local nameTag=Instance.new("TextLabel",previewBg); nameTag.Size=UDim2.new(0,80,0,14); nameTag.Position=UDim2.new(0.5,-40,0,0); nameTag.BackgroundTransparency=1; nameTag.Text="Player"; nameTag.TextColor3=Config.ESP.NameColor; nameTag.Font=Enum.Font.GothamBold; nameTag.TextSize=11; nameTag.TextXAlignment=Enum.TextXAlignment.Center; nameTag.ZIndex=9
    local hpBg=Instance.new("Frame",previewBg); hpBg.Size=UDim2.new(0,4,0,116); hpBg.Position=UDim2.new(0.5,-26,0,12); hpBg.BackgroundColor3=Color3.fromRGB(20,20,20); hpBg.BorderSizePixel=0; hpBg.ZIndex=9
    local hpFill=Instance.new("Frame",hpBg); hpFill.Size=UDim2.new(1,0,0.8,0); hpFill.Position=UDim2.new(0,0,0.2,0); hpFill.BackgroundColor3=Color3.fromRGB(50,220,80); hpFill.BorderSizePixel=0; hpFill.ZIndex=10
    local distTag=Instance.new("TextLabel",previewBg); distTag.Size=UDim2.new(0,80,0,12); distTag.Position=UDim2.new(0.5,-40,1,-28); distTag.BackgroundTransparency=1; distTag.Text="42m"; distTag.TextColor3=Color3.fromRGB(200,200,200); distTag.Font=Enum.Font.Gotham; distTag.TextSize=10; distTag.TextXAlignment=Enum.TextXAlignment.Center; distTag.ZIndex=9
    local tracer=Instance.new("Frame",previewBg); tracer.Size=UDim2.new(0,1,0,30); tracer.Position=UDim2.new(0.5,0,1,-30); tracer.BackgroundColor3=Config.ESP.TracerColor; tracer.BorderSizePixel=0; tracer.ZIndex=9
    local skelLines={}
    local function SkelLine(x1f,y1,x2f,y2)
        local ln=Instance.new("Frame",previewBg); local cx=(x1f+x2f)/2; local cy=(y1+y2)/2
        local len=math.sqrt(((x2f-x1f)*38)^2+(y2-y1)^2)
        ln.Size=UDim2.new(0,math.max(1,math.floor(len)),0,1); ln.Position=UDim2.new(0.5,math.floor(cx)-math.floor(len/2),0,math.floor(cy))
        ln.BackgroundColor3=Config.ESP.SkeletonColor; ln.BorderSizePixel=0; ln.ZIndex=9; table.insert(skelLines,ln); return ln
    end
    SkelLine(0,20,0,92); SkelLine(-0.5,60,0.5,60); SkelLine(-0.3,60,-0.5,84); SkelLine(0.3,60,0.5,84); SkelLine(-0.15,20,-0.25,54); SkelLine(0.15,20,0.25,54)
    local previewConn; previewConn=RunService.RenderStepped:Connect(function()
        if not previewRow.Parent then previewConn:Disconnect(); return end
        ShowBox(Config.ESP.ShowBox); SetBoxColor(Config.ESP.BoxColor)
        ...
        for _,ln in ipairs(skelLines) do ln.Visible=Config.ESP.ShowSkeleton; ln.BackgroundColor3=Config.ESP.SkeletonColor end
        previewBg.BackgroundColor3=Config.ESP.ShowFill and Color3.fromRGB(math.floor(Config.ESP.FillColor.R*255*0.3+30*0.7),math.floor(Config.ESP.FillColor.G*255*0.3+35*0.7),math.floor(Config.ESP.FillColor.B*255*0.3+45*0.7)) or Color3.fromRGB(30,35,45)
    end)
    ...
    MakeColorPicker(s,"Skeleton",Config.ESP.SkeletonColor,function(c)
        Config.ESP.SkeletonColor=c; for _,e in pairs(ESPObjects) do if e.Skeleton then for _,ln in ipairs(e.Skeleton) do ln.Color=c end end end
    end,37)
...
local aimbotPage=MakePage(function(s)
    ...
    -- Smoothing slider: 1=instant, higher=smoother
    MakeSlider(s,"Smoothing (1=instant)",1,30,1,function(v) Config.Aimbot.Smoothing=v end,7)
...
local infoPage=MakePage(function(s)
    ...
    local function InfoRow(parent,labelText,value,order)
        local row=Instance.new("Frame",parent)
        row.Size=UDim2.new(1,0,0,34); row.BackgroundColor3=M.Win
        row.BorderSizePixel=0; row.LayoutOrder=order; row.ZIndex=5; MakeSep(row)
        local lbl=Instance.new("TextLabel",row)
...
local setPage=MakePage(function(s)
    MakeSection(s,"ACCENT COLOUR",1)
    MakeColorPicker(s,"Accent",M.Accent,function(col) ApplyAccent(col) end,2)
end)
...
UserInputService.InputBegan:Connect(function(input,gpe)
    ...
    if input.KeyCode==Enum.KeyCode.RightShift then
        _guiVisible=not _guiVisible; Window.Visible=_guiVisible
    end
end)
...
task.spawn(function()
    ...
    if GM.GetAimSpeed then W.OriginalGetAimSpeed=GM.GetAimSpeed; GM.GetAimSpeed=function(self) if W.InstantADS then return 999 end; return W.OriginalGetAimSpeed(self) end end
    if GM.Equip then
...
task.spawn(function()
    ...
    GU.GetSpread=function(sp,am,ia,ic,pl,tot,con)
        if W.NoSpread then return CFrame.new() end; return W.OriginalGetSpread(sp,am,ia,ic,pl,tot,con)
    end
end)
...
task.spawn(function()
    ...
    VM.new=function(...)
        ...
        if vm.Update then
            ...
            vm.Update=function(self,...) if W.NoWeaponBob then if self.BobSpeed then self.BobSpeed=0 end; if self.BobIntensity then self.BobIntensity=0 end end; return origUpd(self,...) end
        end
        return vm
    end
end)
...
task.spawn(function()
    ...
    for _,fn in ipairs(paths) do
        local ok,mod=pcall(fn); if ok and mod then RivalsCharModule=mod; break end
    end
    if RivalsCharModule then
        if RivalsCharModule.GetSpeed then
            RivalsOrigGetSpeed=RivalsCharModule.GetSpeed
            RivalsCharModule.GetSpeed=function(self,...) if Config.Movement.SpeedEnabled and Config.Movement.SpeedMode=="Rivals" then return Config.Movement.Speed end; return RivalsOrigGetSpeed(self,...) end
        end
        if RivalsCharModule.GetJumpPower then
            RivalsOrigGetJump=RivalsCharModule.GetJumpPower
            RivalsCharModule.GetJumpPower=function(self,...) if Config.Movement.JumpEnabled and Config.Movement.JumpMode=="Rivals" then return Config.Movement.JumpPower end; return RivalsOrigGetJump(self,...) end
        end
    end
end)

-- ============================================================
--  ESP SKELETON / CREATE / REMOVE
-- ============================================================
...
local function CreateESP(player)
    if player==LocalPlayer then return end; RemoveESP(player)
    local sk={}
    for i=1,#BONES do local ln=Drawing.new("Line"); ln.Visible=false; ln.Thickness=1; ln.Color=Config.ESP.SkeletonColor; sk[i]=ln end
    local e={Box=Drawing.new("Square"),BoxFill=Drawing.new("Square"),Name=Drawing.new("Text"),Tracer=Drawing.new("Line"),HealthBarBG=Drawing.new("Square"),HealthBarFill=Drawing.new("Square"),Distance=Drawing.new("Text"),Item=Drawing.new("Text"),Skeleton=sk}
...
for _,p in ipairs(Players:GetPlayers()) do if p~=LocalPlayer then p.CharacterAdded:Connect(function() task.wait(0.1); CreateESP(p) end) end end
...
local function StartFly()
    ...
    flyConn=RunService.RenderStepped:Connect(function()
        ...
        bp.Velocity=dir.Magnitude>0 and dir.Unit*Config.Movement.FlySpeed or Vector3.zero; bg.CFrame=Camera.CFrame
    end)
end
local _pFly=false
RunService.Heartbeat:Connect(function() if Config.Movement.Fly~=_pFly then _pFly=Config.Movement.Fly; if Config.Movement.Fly then StartFly() end end end)

-- ============================================================
--  FOV DRAWINGS
-- ============================================================
local FovFill=Drawing.new("Circle"); FovFill.Visible=false; FovFill.Radius=Config.Aimbot.FovRadius; FovFill.Color=Color3.fromRGB(255,255,255); FovFill.Transparency=0.2; FovFill.Thickness=1; FovFill.Filled=true
local FovCircle=Drawing.new("Circle"); FovCircle.Visible=false; FovCircle.Radius=Config.Aimbot.FovRadius; FovCircle.Color=Color3.fromRGB(205,205,205); FovCircle.Thickness=1; FovCircle.Filled=false
TargetTracer=Drawing.new("Line"); TargetTracer.Visible=false; TargetTracer.Color=Config.Aimbot.TracerColor; TargetTracer.Thickness=1; TargetTracer.Transparency=1
local FovCirclePos=UserInputService:GetMouseLocation()

-- ============================================================
--  CHARACTER TP STATE
-- ============================================================
local charTPActive=false; local charTPOrigPos=nil
local function CharTPStop()
    if not charTPActive then return end; charTPActive=false
    local myChar=LocalPlayer.Character; local myRoot=myChar and myChar:FindFirstChild("HumanoidRootPart")
    if myRoot and charTPOrigPos then pcall(function() myRoot.CFrame=charTPOrigPos end) end
    charTPOrigPos=nil
end
...
RunService.RenderStepped:Connect(function()
    ...
    for player,d in pairs(ESPObjects) do
        ...
        if Config.ESP.Chams and ok then
            if not ChamObjects[player] then
                local hl=Instance.new("Highlight"); hl.FillColor=Config.ESP.ChamsColor; hl.FillTransparency=Config.ESP.ChamsTransp; hl.OutlineColor=Config.ESP.ChamsColor; hl.OutlineTransparency=0.3; hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop; hl.Adornee=char; hl.Parent=char; ChamObjects[player]=hl
            end
        ...
        local dist=(Camera.CFrame.Position-root.Position).Magnitude; if dist>Config.ESP.MaxDist then hideAll(); continue end
        local rp,onS=Camera:WorldToViewportPoint(root.Position); if not onS then hideAll(); continue end
        local tp2=Camera:WorldToViewportPoint(root.Position+Vector3.new(0,3.2,0)); local bp3=Camera:WorldToViewportPoint(root.Position-Vector3.new(0,3.2,0))
        local bH=math.abs(tp2.Y-bp3.Y); local bW=bH*0.55; local bX=rp.X-bW/2; local bY=tp2.Y
        ...
        if Config.ESP.ShowName then local np=Camera:WorldToViewportPoint(root.Position+Vector3.new(0,3.8,0)); d.Name.Text=player.Name; d.Name.Position=Vector2.new(np.X,np.Y); d.Name.Color=Config.ESP.NameColor; d.Name.Visible=true else d.Name.Visible=false end
        if Config.ESP.ShowTracer then d.Tracer.From=Vector2.new(Camera.ViewportSize.X/2,Camera.ViewportSize.Y); d.Tracer.To=Vector2.new(rp.X,rp.Y); d.Tracer.Color=Config.ESP.TracerColor; d.Tracer.Visible=true else d.Tracer.Visible=false end
        local textY=bY+bH+2
        ...
        if Config.ESP.ShowItem then local tool=char:FindFirstChildOfClass("Tool"); if tool then d.Item.Text="🔧 "..tool.Name; d.Item.Position=Vector2.new(rp.X,textY); d.Item.Visible=true else d.Item.Visible=false end else d.Item.Visible=false end
        if Config.ESP.ShowSkeleton then
            for i,pair in ipairs(BONES) do
                ...
                if pA and pB then local sA,okA=Camera:WorldToViewportPoint(pA.Position); local sB,okB=Camera:WorldToViewportPoint(pB.Position); if okA and okB then ln.From=Vector2.new(sA.X,sA.Y); ln.To=Vector2.new(sB.X,sB.Y); ln.Color=Config.ESP.SkeletonColor; ln.Visible=true else ln.Visible=false end
    ...
    -- ── AIMBOT ─────────────────────────────────────────────
    -- FIX: instant lock when smoothing=1 (no division dampening)
        ...
        if tdata then
            ...
            if onS then
                ...
                if Config.Aimbot.AimType=="MouseMove" then
                    ...
                    if sm==1 then
                        -- Instant: move the full delta in one frame
                        mousemoverel(delta.X, delta.Y)
                    else
                        mousemoverel(math.clamp(delta.X/sm,-80,80), math.clamp(delta.Y/sm,-80,80))
                    end
                else
                    if sm==1 then
                        -- Instant: snap directly to target
                        Camera.CFrame=CFrame.new(Camera.CFrame.Position,target.Position)
                    else
                        Camera.CFrame=Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position,target.Position),1/sm)
                    end
                end
            ...
            if Config.Aimbot.CharTP then
                local targetRoot=tdata.char:FindFirstChild("HumanoidRootPart")
                if targetRoot then
                    local myChar=LocalPlayer.Character; local myRoot=myChar and myChar:FindFirstChild("HumanoidRootPart")
                    if myRoot then
                        if not charTPActive then charTPOrigPos=myRoot.CFrame; charTPActive=true end
                        pcall(function() myRoot.CFrame=CFrame.new(targetRoot.Position+Vector3.new(0,9,0),targetRoot.Position+Vector3.new(0,9,0)+Camera.CFrame.LookVector) end)
                    end
                end
            end
        else
...
task.spawn(function()
    ...
    GM.StartShooting=function(self,p1,p2)
        if SilentAimActive() then
            local tgt=GetSilentTarget(); if tgt then
                local realCF=Camera.CFrame; _silentCF=CFrame.new(realCF.Position,tgt.Position+Vector3.new(0,0.15,0)); _silentActive=true
                local res={prev(self,p1,p2)}; _silentActive=false; _silentCF=nil; return unpack(res)
            end
        end
...
local function ClearNoclipConns() for _,c in ipairs(noclipConns) do pcall(function() c:Disconnect() end) end; noclipConns={} end
local function RestoreCollision() local char=LocalPlayer.Character; if not char then return end; for _,p in ipairs(char:GetDescendants()) do if p:IsA("BasePart") then pcall(function() p.CanCollide=true end) end end end
local function DisableNoclip() ClearNoclipConns(); if _normalNoclipConn then pcall(function() _normalNoclipConn:Disconnect() end); _normalNoclipConn=nil end; RestoreCollision() end
local function HookPart(part)
    if not part:IsA("BasePart") then return end; pcall(function() part.CanCollide=false end)
    local ok,conn=pcall(function() return part:GetPropertyChangedSignal("CanCollide"):Connect(function() if Config.Movement.Noclip and Config.Movement.NoclipMode=="Rivals" then pcall(function() part.CanCollide=false end) end end) end)
...
local function StopSpeed() if _speedConn then pcall(function() _speedConn:Disconnect() end); _speedConn=nil end; local hum=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid"); if hum then pcall(function() hum.WalkSpeed=16 end) end end
local function StartSpeedLoop()
    ...
    _speedConn=RunService.Heartbeat:Connect(function()
        ...
        if Config.Movement.SpeedMode=="Rivals" then
            ...
            if root then local vel=root.AssemblyLinearVelocity; local flat=Vector3.new(vel.X,0,vel.Z); if flat.Magnitude>0.5 then local nf=flat.Unit*Config.Movement.Speed; pcall(function() root.AssemblyLinearVelocity=Vector3.new(nf.X,vel.Y,nf.Z) end) end end
        else pcall(function() hum.WalkSpeed=Config.Movement.Speed end) end
    end)
end

local function StopJump() if _jumpConn then pcall(function() _jumpConn:Disconnect() end); _jumpConn=nil end; local hum=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid"); if hum then pcall(function() hum.JumpPower=50; hum.JumpHeight=7.2 end) end end
local function StartJumpLoop()
    ...
    _jumpConn=RunService.Heartbeat:Connect(function()
        ...
        local char=LocalPlayer.Character; if not char then return end
        local hum=char:FindFirstChild("Humanoid"); if not hum then return end
        pcall(function() hum.JumpPower=Config.Movement.JumpPower; hum.JumpHeight=Config.Movement.JumpPower*0.56
            if Config.Movement.JumpMode=="Rivals" then local root=char:FindFirstChild("HumanoidRootPart"); if root and hum:GetState()==Enum.HumanoidStateType.Jumping then local vel=root.AssemblyLinearVelocity; if vel.Y>0 and vel.Y<Config.Movement.JumpPower then pcall(function() root.AssemblyLinearVelocity=Vector3.new(vel.X,Config.Movement.JumpPower*0.5,vel.Z) end) end end end
        end)
    end)
end
...
local function SetupInfJump(char)
    local hum=char:FindFirstChild("Humanoid"); if not hum then return end
    hum.StateChanged:Connect(function(old,new) if new==Enum.HumanoidStateType.Landed and Config.Movement.InfJump then hum:SetStateEnabled(Enum.HumanoidStateType.Jumping,true) end end)
end
...
UserInputService.JumpRequest:Connect(function()
    ...
    local root=char:FindFirstChild("HumanoidRootPart"); if root then pcall(function() local vel=root.AssemblyLinearVelocity; root.AssemblyLinearVelocity=Vector3.new(vel.X,math.max(vel.Y,40),vel.Z) end) end
end)
...
-- ============================================================
--  3RD PERSON / DEVICE SPOOFER
-- ============================================================
local _tpConn=nil
local function EnableThirdPerson()
    if _tpConn then pcall(function() _tpConn:Disconnect() end) end
    _tpConn=RunService.RenderStepped:Connect(function()
        if not Config.Rivals.ThirdPerson then pcall(function() _tpConn:Disconnect() end); _tpConn=nil; pcall(function() LocalPlayer.CameraMaxZoomDistance=400; LocalPlayer.CameraMinZoomDistance=0.5 end); return end
        local char=LocalPlayer.Character; if not char then return end; local hum=char:FindFirstChild("Humanoid"); if not hum then return end
        pcall(function() Camera.CameraType=Enum.CameraType.Custom; Camera.CameraSubject=hum; LocalPlayer.CameraMaxZoomDistance=Config.Rivals.ThirdPersonDist; LocalPlayer.CameraMinZoomDistance=0.5 end)
    end)
end
local _pTP=false
RunService.Heartbeat:Connect(function()
    if Config.Rivals.ThirdPerson~=_pTP then _pTP=Config.Rivals.ThirdPerson; if _pTP then EnableThirdPerson() else if _tpConn then pcall(function() _tpConn:Disconnect() end); _tpConn=nil end; pcall(function() LocalPlayer.CameraMaxZoomDistance=400; LocalPlayer.CameraMinZoomDistance=0.5 end) end end
end)

local _spoofMap={}; local _devHooked=false; local _origUISIdx=nil
local function BuildSpoofMap(dt) if dt=="Mobile" then _spoofMap={TouchEnabled=true,GamepadEnabled=false,KeyboardEnabled=false,MouseEnabled=false} elseif dt=="Console" then _spoofMap={TouchEnabled=false,GamepadEnabled=true,KeyboardEnabled=false,MouseEnabled=false} else _spoofMap={TouchEnabled=false,GamepadEnabled=false,KeyboardEnabled=true,MouseEnabled=true} end end
local function InstallDeviceSpoof() if _devHooked then return end; local ok,mt=pcall(getrawmetatable,UserInputService); if not ok or not mt then return end; local oldIdx=rawget(mt,"__index"); if not oldIdx then return end; _origUISIdx=oldIdx; pcall(function() setreadonly(mt,false); mt.__index=newcclosure(function(self,key) if Config.Rivals.DeviceSpoofEnabled and _spoofMap[key]~=nil then return _spoofMap[key] end; return oldIdx(self,key) end); setreadonly(mt,true) end); _devHooked=true end
local function RemoveDeviceSpoof() if not _origUISIdx then return end; local ok,mt=pcall(getrawmetatable,UserInputService); if not ok or not mt then return end; pcall(function() setreadonly(mt,false); mt.__index=_origUISIdx; setreadonly(mt,true) end); _origUISIdx=nil; _devHooked=false end
RunService.Heartbeat:Connect(function() if Config.Rivals.DeviceSpoofEnabled then BuildSpoofMap(Config.Rivals.DeviceType); if not _devHooked then pcall(InstallDeviceSpoof) end end end)

-- ============================================================
--  RESPAWN
-- ============================================================
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(0.5); local hum=char:WaitForChild("Humanoid")
...
local function DestroyAll()
    ...
    if RivalsCharModule then if RivalsOrigGetSpeed then RivalsCharModule.GetSpeed=RivalsOrigGetSpeed end; if RivalsOrigGetJump then RivalsCharModule.GetJumpPower=RivalsOrigGetJump end end
    if W.GunModule then local G=W.GunModule; if W.OriginalStartShooting then G.StartShooting=W.OriginalStartShooting end; if W.OriginalStartReloading then G.StartReloading=W.OriginalStartReloading end; if W.OriginalRecoil then G._Recoil=W.OriginalRecoil end; if W.OriginalStartAiming then G.StartAiming=W.OriginalStartAiming end; if W.OriginalGetAimSpeed then G.GetAimSpeed=W.OriginalGetAimSpeed end; if W.OriginalEquip then G.Equip=W.OriginalEquip end; if W.OriginalLocalTracers then G._LocalTracers=W.OriginalLocalTracers end; if W.OriginalProjectileEffect then G._ProjectileEffect=W.OriginalProjectileEffect end end
    if W.GameplayUtility and W.OriginalGetSpread then W.GameplayUtility.GetSpread=W.OriginalGetSpread end
    for _,e in pairs(ESPObjects) do for _,key in ipairs({"Box","BoxFill","Name","Tracer","HealthBarBG","HealthBarFill","Distance","Item"}) do pcall(function() e[key]:Remove() end) end; if e.Skeleton then for _,ln in ipairs(e.Skeleton) do pcall(function() ln:Remove() end) end end end
    ESPObjects={}; for _,hl in pairs(ChamObjects) do pcall(function() hl:Destroy() end) end; ChamObjects={}
    _silentActive=false; _silentCF=nil
    pcall(function() if _origCamIdx then local mt=getrawmetatable(Camera); if mt then setreadonly(mt,false); mt.__index=_origCamIdx; setreadonly(mt,true) end; _origCamIdx=nil end end)
    pcall(function() FovFill:Remove() end); pcall(function() FovCircle:Remove() end); pcall(function() TargetTracer:Remove() end)
    ScreenGui:Destroy()
end
