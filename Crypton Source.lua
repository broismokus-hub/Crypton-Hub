...
local Config = {
    ...
    Movement = {
        SpeedEnabled=false, Speed=16, SpeedMode="Normal",
        JumpEnabled=false, JumpPower=50, JumpMode="Normal",
...
-- ============================================================
local M = {
    Win        = Color3.fromRGB(236, 236, 236),   -- window bg (light grey)
    Sidebar    = Color3.fromRGB(210, 210, 215),   -- sidebar bg
    TitleBar   = Color3.fromRGB(220, 220, 223),   -- title bar
    Separator  = Color3.fromRGB(185, 185, 190),   -- separator lines
    Row        = Color3.fromRGB(236, 236, 236),   -- normal row
    RowHover   = Color3.fromRGB(210, 225, 248),   -- selected row (blue tint)
    RowActive  = Color3.fromRGB(9,   101, 224),   -- active/selected row (macOS blue)
    TextDark   = Color3.fromRGB(30,  30,  30),    -- primary text
    TextMid    = Color3.fromRGB(90,  90,  100),   -- secondary text
    TextLight  = Color3.fromRGB(150, 150, 160),   -- dimmed text
    TextWhite  = Color3.fromRGB(255, 255, 255),
    Accent     = Color3.fromRGB(9,   101, 224),   -- macOS blue
    AccentGlow = Color3.fromRGB(50,  130, 240),
    Red        = Color3.fromRGB(255, 59,  48),    -- traffic light red
    Yellow     = Color3.fromRGB(255, 189, 46),    -- traffic light yellow
    Green      = Color3.fromRGB(40,  200, 64),    -- traffic light green
    Content    = Color3.fromRGB(245, 245, 247),   -- content area bg
    Shadow     = Color3.fromRGB(100, 100, 110),
    ToggleOff  = Color3.fromRGB(190, 190, 195),
    ToggleOn   = Color3.fromRGB(52,  199, 89),    -- iOS-style green toggle
    SliderFill = Color3.fromRGB(9,   101, 224),
...
local function Stroke(parent, col, th)
    ...
    s.Color = col or M.Border; s.Thickness = th or 1
    return s
end
...
-- ============================================================
--  WINDOW  (macOS Finder proportions: sidebar + content)
-- ============================================================
...
TitleFix.BackgroundColor3 = M.TitleBar
...
TitleSep.BackgroundColor3 = M.Separator
...
local function TrafficBtn(parent, color, xOff)
    ...
    btn.Position         = UDim2.new(0, xOff, 0.5, -6)
    btn.BackgroundColor3 = color
    btn.Text             = ""
    btn.BorderSizePixel  = 0
    btn.ZIndex           = 5
    R(btn, 99)
    Stroke(btn, Color3.new(0,0,0), 0.3)
    return btn
end
...
TitleLbl.Text                   = "Crypton Hub Gui"
TitleLbl.TextColor3             = M.TextDark
TitleLbl.Font                   = Enum.Font.GothamBold
TitleLbl.TextSize               = 13
TitleLbl.TextXAlignment         = Enum.TextXAlignment.Center
TitleLbl.ZIndex                 = 4

-- Toolbar icons (decorative, right side)
local function ToolbarBtn(parent, icon, xFromRight)
    local b = Instance.new("TextButton", parent)
    b.Size             = UDim2.new(0, 24, 0, 22)
    b.Position         = UDim2.new(1, xFromRight, 0.5, -11)
    b.BackgroundColor3 = Color3.fromRGB(200,200,205)
    b.Text             = icon
    b.TextColor3       = M.TextMid
...
ToolbarBtn(TitleBar, "⚙", -32)
ToolbarBtn(TitleBar, "↑↓", -60)
...
Sidebar.BackgroundColor3 = M.Sidebar
Sidebar.BorderSizePixel  = 0
Sidebar.ZIndex           = 3
R(Sidebar, 12)

-- Fix top-right and top-left of sidebar
local SBTopFix = Instance.new("Frame", Sidebar)
SBTopFix.Size             = UDim2.new(1, 0, 0, 12)
SBTopFix.BackgroundColor3 = M.Sidebar
...
SBSep.BackgroundColor3 = M.Separator
...
ContentArea.Size             = UDim2.new(1, -SIDEBAR_W, 1, -38)
ContentArea.Position         = UDim2.new(0, SIDEBAR_W, 0, 38)
ContentArea.BackgroundColor3 = M.Content
ContentArea.BorderSizePixel  = 0
ContentArea.ZIndex           = 3
R(ContentArea, 12)

local CATopFix = Instance.new("Frame", ContentArea)
CATopFix.Size             = UDim2.new(1, 0, 0, 12)
CATopFix.BackgroundColor3 = M.Content
CATopFix.BorderSizePixel  = 0
CATopFix.ZIndex           = 3
...
local function MakeSection(parent, text, order)
    ...
    f.BackgroundColor3 = M.SectionBg
    f.BorderSizePixel  = 0
...
local function MakeToggle(parent, label, default, callback, order)
    ...
    sep.BackgroundColor3 = M.Separator
...
local function MakeSlider(parent, label, min, max, default, callback, order)
    ...
    trackBg.BackgroundColor3 = M.SliderBg
    ...
    fill.BackgroundColor3 = M.SliderFill
    fill.BorderSizePixel  = 0
    fill.ZIndex           = 7
    R(fill, 99)

    local dragging = false
    local hit = Instance.new("TextButton", trackBg)
    ...
    local function apply(ax)
        local rel = math.clamp((ax - trackBg.AbsolutePosition.X)/trackBg.AbsoluteSize.X, 0, 1)
...
local function MakeCycle(parent, label, options, default, callback, order)
    ...
    cycleBtn.Size             = UDim2.new(0, 106, 0, 22)
    cycleBtn.Position         = UDim2.new(1, -116, 0.5, -11)
    cycleBtn.BackgroundColor3 = Color3.fromRGB(220,220,225)
    cycleBtn.Text             = default .. "  ▾"
    cycleBtn.TextColor3       = M.TextDark
    cycleBtn.Font             = Enum.Font.Gotham
    cycleBtn.TextSize         = 12
    cycleBtn.BorderSizePixel  = 0
    cycleBtn.ZIndex           = 7
    R(cycleBtn, 6)
    Stroke(cycleBtn, M.Border, 1)
...
local function MakeKeybind(parent, label, default, callback, order)
    ...
    keyBtn.BackgroundColor3 = Color3.fromRGB(220,220,225)
    keyBtn.Text             = tostring(default):gsub("Enum.KeyCode.","")
    keyBtn.TextColor3       = M.Accent
    keyBtn.Font             = Enum.Font.GothamBold
    keyBtn.TextSize         = 12
    keyBtn.BorderSizePixel  = 0
    keyBtn.ZIndex           = 7
    R(keyBtn, 6)
    Stroke(keyBtn, M.Border, 1)

    local listening = false
    keyBtn.MouseButton1Click:Connect(function()
        if listening then return end
        listening = true
        keyBtn.Text = "..."; keyBtn.TextColor3 = Color3.fromRGB(255,150,0)
        local conn
        conn = UserInputService.InputBegan:Connect(function(input, gpe)
            if gpe then return end
            local skip = {[Enum.KeyCode.LeftShift]=1,[Enum.KeyCode.RightShift]=1,[Enum.KeyCode.LeftControl]=1,[Enum.KeyCode.RightControl]=1}
...
local function MakeColorPicker(parent, label, defaultColor, callback, startOrder)
    ...
    hdr.BackgroundColor3 = M.SectionBg
    hdr.BorderSizePixel  = 0
    hdr.LayoutOrder      = startOrder or 0
    hdr.ZIndex           = 5
    ...
    local channels = {
        ...
        {name="B", getV=function() return cb end, setV=function(v) cb=v end, col=Color3.fromRGB(60,120,220), def=b0, order=startOrder+3},
    }
    for _, ch in ipairs(channels) do
        ...
        track.Size             = UDim2.new(1,-62,0,4)
        track.Position         = UDim2.new(0,30,0.5,-2)
        track.BackgroundColor3 = M.SliderBg
        track.BorderSizePixel  = 0
        track.ZIndex           = 6
        R(track, 99)

        local fill2 = Instance.new("Frame", track)
        fill2.Size             = UDim2.new(ch.def/255,0,1,0)
        fill2.BackgroundColor3 = ch.col
...
local function AddNavEntry(icon, name, page)
    ...
    if pageOrder == 1 then
        ...
        btn.BackgroundColor3       = M.RowActive
        nameL.TextColor3           = M.TextWhite
        iconL.TextColor3           = M.TextWhite
        page.Visible               = true
        ActivePage   = page
        ActiveNavBtn = {btn=btn, nameL=nameL, iconL=iconL}
    end
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if ActiveNavBtn then
            ...
            ActiveNavBtn.nameL.TextColor3           = M.TextDark
            ActiveNavBtn.iconL.TextColor3           = M.Accent
        end
...
-- sidebar section label (FAVORITES / DEVICES style)
...
-- Sidebar section labels (rendered before nav entries due to LayoutOrder)
SBSection("VISUALS", 0)
...
local espPage = MakePage(function(s)
    ...
    MakeToggle(s, "Chams",         false, function(v)
        Config.ESP.Chams = v
        if not v then
            for plr, hl in pairs(ChamObjects) do pcall(function() hl:Destroy() end); ChamObjects[plr]=nil end
        end
    end, 11)
    MakeSection(s, "COLOURS", 12)
    MakeColorPicker(s, "Box",      Config.ESP.BoxColor,      function(c) Config.ESP.BoxColor=c; for _,e in pairs(ESPObjects) do if e.Box    then e.Box.Color=c    end end end, 13)
    MakeColorPicker(s, "Fill",     Config.ESP.FillColor,     function(c) Config.ESP.FillColor=c; for _,e in pairs(ESPObjects) do if e.BoxFill then e.BoxFill.Color=c end end end, 17)
    MakeColorPicker(s, "Name",     Config.ESP.NameColor,     function(c) Config.ESP.NameColor=c; for _,e in pairs(ESPObjects) do if e.Name   then e.Name.Color=c   end end end, 21)
    MakeColorPicker(s, "Tracer",   Config.ESP.TracerColor,   function(c) Config.ESP.TracerColor=c; for _,e in pairs(ESPObjects) do if e.Tracer then e.Tracer.Color=c end end end, 25)
    ...
    MakeColorPicker(s, "Chams",    Config.ESP.ChamsColor,    function(c)
        Config.ESP.ChamsColor=c
        for _,hl in pairs(ChamObjects) do hl.FillColor=c; hl.OutlineColor=c end
    end, 33)
end)
...
local aimbotPage = MakePage(function(s)
    ...
    MakeCycle(s,   "Aim Type", {"MouseMove","CFrame"}, "MouseMove", function(v) Config.Aimbot.AimType=v end, 4)
    MakeKeybind(s, "Lock Key", Enum.KeyCode.X, function(k)
        Config.Aimbot.Key=k
        secLbl.Text="AIMBOT  —  hold "..tostring(k):gsub("Enum.KeyCode.","").." to lock on"
    end, 5)
    MakeSlider(s,  "Smoothing",   1,  30,  10,  function(v) Config.Aimbot.Smoothing  = v end, 6)
    MakeSlider(s,  "FOV Radius",  10, 800, 150, function(v) Config.Aimbot.FovRadius  = v end, 7)
    MakeSection(s, "SILENT AIM", 8)
    MakeToggle(s,  "Silent Aim",        false, function(v) Config.Aimbot.SilentAim    = v end, 9)
    MakeToggle(s,  "Silent FOV",        false, function(v) Config.Aimbot.SilentAimFov = v end, 10)
    MakeSlider(s,  "Silent FOV Radius", 10, 800, 150, function(v) Config.Aimbot.SilentAimFovRadius=v end, 11)
    ...
    MakeKeybind(s, "Silent Key", Enum.KeyCode.Z, function(k) Config.Aimbot.SilentAimKey=k end, 13)
    MakeSection(s, "COLOURS", 14)
    MakeColorPicker(s, "Tracer Colour", Color3.fromRGB(255,60,60), function(c)
        Config.Aimbot.TracerColor=c
        if TargetTracer then TargetTracer.Color=c end
    end, 15)
end)
...
local movPage = MakePage(function(s)
    ...
    MakeCycle(s,  "Speed Mode", {"Normal","Rivals"}, "Normal", function(v) Config.Movement.SpeedMode=v end, 3)
    MakeSlider(s, "Walk Speed", 16, 200, 16, function(v) Config.Movement.Speed=v end, 4)
    MakeSection(s, "JUMP POWER", 5)
    MakeToggle(s, "Enable Jump",   false, function(v) Config.Movement.JumpEnabled=v end, 6)
    MakeCycle(s,  "Jump Mode", {"Normal","Rivals"}, "Normal", function(v) Config.Movement.JumpMode=v end, 7)
    MakeSlider(s, "Jump Power", 50, 300, 50, function(v) Config.Movement.JumpPower=v end, 8)
...
local infoPage = MakePage(function(s)
    ...
    local function InfoRow(parent, labelText, value, order)
        ...
        val.Size             = UDim2.new(0.52,-12,1,0)
        val.Position         = UDim2.new(0.48,0,0,0)
        val.BackgroundTransparency = 1
        val.Text             = tostring(value)
        val.TextColor3       = M.Accent
    ...
    local av = InfoRow(s, "Account Age", "", 4)
    av.Text = LocalPlayer.AccountAge.."d"
...
local setPage = MakePage(function(s)
    MakeSection(s, "MENU COLOUR", 1)
    MakeColorPicker(s, "Accent", M.Accent, function(col)
        M.Accent = col
        -- propagate to nav rows and accent text
        for _, desc in ipairs(ScreenGui:GetDescendants()) do
            pcall(function()
                if (desc:IsA("TextLabel") or desc:IsA("TextButton")) then
                    if desc.TextColor3 == M.SliderFill or desc.Name == "AccentText" then
                        desc.TextColor3 = col
                    end
                end
            end)
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        -- hide content, show only title bar
        Sidebar.Visible     = false
        ContentArea.Visible = false
        Window.Size         = UDim2.new(0, WIN_W, 0, 38)
        Shadow.Size         = UDim2.new(0, WIN_W+8, 0, 46)
    else
...
-- ============================================================
--  RSHIFT TOGGLE (hide/show entire GUI)
-- ============================================================
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.RightShift then
        ...
        Window.Visible = _guiVisible
        Shadow.Visible = _guiVisible
    end
end)

-- Scroll to resize FOV when aimbot key held
...
-- ============================================================
--  RIVALS WEAPON HOOKS  (identical to original)
-- ============================================================
...
RunService.RenderStepped:Connect(function()
    for player, d in pairs(ESPObjects) do
        ...
        if Config.ESP.ShowHealthBar then
            ...
            local fH=math.max(1,bH*pct)
            d.HealthBarFill.Size=Vector2.new(4,fH); d.HealthBarFill.Position=Vector2.new(bx2,bY+bH-fH); d.HealthBarFill.Transparency=1
            d.HealthBarFill.Color=pct>0.6 and Color3.fromRGB(50,220,80) or (pct>0.3 and Color3.fromRGB(230,180,40) or Color3.fromRGB(220,50,50))
    ...
    local function GetTarget(fovR, useMouse)
        local ref=useMouse and mousePos or center
    ...
    if Config.Aimbot.Enabled and UserInputService:IsKeyDown(Config.Aimbot.Key) then
        local target=GetTarget(Config.Aimbot.FovRadius, true)
        if target then
...
local function StartSpeedLoop()
    ...
    _speedConn=RunService.Heartbeat:Connect(function()
        ...
        local hum=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
        if hum then pcall(function() hum.WalkSpeed=Config.Movement.Speed end) end
    end)
end
...
local function StartJumpLoop()
    ...
    _jumpConn=RunService.Heartbeat:Connect(function()
        ...
        local hum=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
        if hum then
            pcall(function() hum.JumpPower=Config.Movement.JumpPower end)
            if Config.Movement.JumpMode=="Rivals" then pcall(function() hum.JumpHeight=Config.Movement.JumpPower*0.56 end) end
        end
    end)
end
...
RunService.Heartbeat:Connect(function()
    if Config.Rivals.DeviceSpoofEnabled then
        BuildSpoofMap(Config.Rivals.DeviceType); if not _devHooked then pcall(InstallDeviceSpoof) end
    end
end)
...
local function DestroyAll()
    ...
    Config.Aimbot.Enabled=false; Config.Rivals.ThirdPerson=false; Config.Rivals.DeviceSpoofEnabled=false
    if _speedConn  then pcall(function() _speedConn:Disconnect()  end); _speedConn=nil  end
    if _jumpConn   then pcall(function() _jumpConn:Disconnect()   end); _jumpConn=nil   end
...
print("[Crypton GUI] Loaded ✓  |  RShift = toggle  |  X (hold) = aimbot  |  RShift+Del = destroy")
